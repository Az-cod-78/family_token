...
class NftrentController extends Controller
{
    public function index(Request $request)
    {
        ...
        $web3 = new Web3(new HttpProvider(new HttpRequestManager('https://bsc-dataseed1.binance.org:443')));
        ...
        if ($request->isMethod('post')) {
            ...
            try {
                // Mint new NFT
                $contract->at($contractAddress)->call('mint', [$name, $price]);

                // Transfer FT token
                $contract->at($contractAddress)->call('transfer', [$to, $value]);
            } catch (\Exception $e) {
                // Handle exception
            }
        }
